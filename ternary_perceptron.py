...
def student_name():
    return "Tommy Zhao"  # Replace with your own name.
...
class TernaryPerceptron:
    ...
    def classify(self, x_vector):
        ...
        for j in range(3):
            ...
            for i in range(len(x_vector)):
                y += (x_vector[i] * self.W[j][i])
            y += self.W[j][-1]
            y_list.append(y)

        return self.argmax(y_list)
    ...
    def train_with_one_example(self, x_vector, y):
        ...
        ---
        Inputs:
        x_vector: Feature vector, same as method classify: [x_0, x_1, ..., x_{n-1}]
        ...
        for i in range(len(x_vector)):
            self.W[y_hat][i] -= self.alpha * x_vector[i]  # decrease misclassified weights
            self.W[y][i] += self.alpha * x_vector[i]  # increase weights of true class
        self.W[y_hat][-1] -= self.alpha

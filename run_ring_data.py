...
class PlotRingData(PlotBinaryPerceptron):
    ...
    def __init__(self, bp, plot_all=True, n_epochs=20, remap=False):
        super().__init__(bp, plot_all, n_epochs)  # Calls the constructor of the super class
        self.IS_REMAPPED = remap
    ...
    def read_data(self):
        ...
        if self.IS_REMAPPED:
            self.TRAINING_DATA = [[remapper.remap(float(f1), float(f2))[0], remapper.remap(float(f1), float(f2))[1],
